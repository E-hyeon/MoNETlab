...
sudo ip netns exec ue1 ip route del default 2>/dev/null || true
...
sudo ip netns exec ue2 ip route del default 2>/dev/null || true
...
sleep 1
sudo ip netns exec ue1 iperf3 -c 192.168.70.135 -B $UE1_IP -u -b 20M -t 3600 --logfile ~/monetlab/iperf_ue1.log &
sudo ip netns exec ue2 iperf3 -c 192.168.70.135 -B $UE2_IP -u -b 20M -p 5202 -t 3600 --logfile ~/monetlab/iperf_ue2.log &

echo "[6/6] KPI collector..."
kill $(ps -ef | grep collect_kpi | grep -v grep | awk '{print $2}') 2>/dev/null || true
rm -f ~/monetlab/kpi_baseline.csv
tail -f ~/monetlab/gnb_live.log | python3 ~/monetlab/collect_kpi.py &

echo "Done! 30초 후 확인하려면: sleep 30 && tail -3 iperf_ue1.log && tail -3 iperf_ue2.log && wc -l kpi_baseline.csv"
