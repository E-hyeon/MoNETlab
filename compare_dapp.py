"""
compare_dapp.py — OAI 기본 / Chronos dApp(TCN 예측) / Reactive Baseline(no AI) / EWMA dApp 4자 비교
...
windows_off_nprb, windows_on_nprb, windows_react_nprb, windows_ewma_nprb = [], [], [], []
windows_off_byte, windows_on_byte, windows_react_byte, windows_ewma_byte = [], [], [], []
...
print("=" * 78)
print(f"  {'UE':>6}  {'SNR':>6}  "
      f"{'nPRB_OFF':>9} {'nPRB_ON':>8} {'nPRB_RCT':>9} {'nPRB_EWMA':>10}  "
      f"{'bytes_OFF':>10} {'bytes_ON':>9} {'bytes_RCT':>10} {'bytes_EWMA':>11}")
for ue in ues:
    snr_real = df[df['rnti'] == ue]['snr'].mean()
    no = np.mean(ue_nprb_off[ue]); nn = np.mean(ue_nprb_on[ue])
    nr = np.mean(ue_nprb_react[ue]); ne = np.mean(ue_nprb_ewma[ue])
    bo = np.mean(ue_byte_off[ue]); bn = np.mean(ue_byte_on[ue])
    br = np.mean(ue_byte_react[ue]); be = np.mean(ue_byte_ewma[ue])
    print(f"  {ue:>6}  {snr_real:>6.1f}  "
          f"{no:>9.1f} {nn:>8.1f} {nr:>9.1f} {ne:>10.1f}  "
          f"{bo:>10.1f} {bn:>9.1f} {br:>10.1f} {be:>11.1f}")
