...
print("=" * 55)
df = pd.read_csv('kpi_live.csv', parse_dates=['timestamp'])
df = df.sort_values('timestamp').reset_index(drop=True)
df[FEATURES] = df[FEATURES].fillna(0)
df['rnti'] = df['rnti'].astype(str)

# 충분한 데이터가 있는 UE만 사용 (최소 50행)
ue_counts = df['rnti'].value_counts()
valid_ues = ue_counts[ue_counts >= 20].index.tolist()
df = df[df['rnti'].isin(valid_ues)].reset_index(drop=True)
print(f"데이터: {len(df)}행  UE: {df['rnti'].nunique()}개 {valid_ues}")

tsdata_train, tsdata_val, tsdata_test = TSDataset.from_pandas(
    df, dt_col='timestamp', id_col='rnti', target_col=FEATURES,
...
print(f"\n학습 시작 (epochs={EPOCHS}, kpi_live.csv 기반)")
...
print(f"MSE: {float(np.mean(mse)):.6f}")
print(f"MAE: {float(np.mean(mae)):.6f}")
print("\nFeature별 MAE (실제 단위):")
for f, e in zip(FEATURES, mae):
    print(f"  {f:12s}: {float(np.mean(e)):.4f}")
